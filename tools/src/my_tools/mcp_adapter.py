...
class MCPClientManager:
    ...
    def _server_params(self, target: str, *, inherit_env: bool = True) -> StdioServerParameters:
        """Build stdio server parameters for either a script path or a module spec.

        Supported target forms:
        - Absolute/relative script path: "/abs/path/mcp_ro.py" or "mcp_ro.py"
        - Module mode: "module:my_tools.mcp_ro" or "my_tools.mcp_ro"
        """
        t = (target or "").strip()
        if not t:
            raise ValueError("empty MCP server target")

        module = ""
        if t.startswith("module:"):
            module = t.split(":", 1)[1].strip()
        else:
            # Heuristic: treat dotted python path as module if it doesn't look like a file path.
            looks_like_path = (
                t.endswith(".py")
                or "/" in t
                or "\\" in t
            ...
            if not looks_like_path and "." in t:
                module = t

        args = ["-m", module] if module else [t]
        return StdioServerParameters(
            command=sys.executable,
            args=args,
            env=os.environ.copy() if inherit_env else None,  # type: ignore[arg-type]
        )
    ...
    async def get_session(self, script_name: str) -> ClientSession:
        ...
        server_params = self._server_params(script_name, inherit_env=True)
    ...
    async def get_tools(self, script_name: str, temporary: bool = True) -> List[StructuredTool]:
        ...
        """
        if temporary:
            # For listing tools we don't need to inherit full env; keep behavior similar to previous code.
            server_params = self._server_params(script_name, inherit_env=False)
