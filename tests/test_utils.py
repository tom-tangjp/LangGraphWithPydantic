...
from utils import env, get_config, _deep_update
...
def test_env_with_config():
    ...
    # Temporarily replace _CONFIG_PATHS
    from utils import _CONFIG_PATHS

    original_paths = _CONFIG_PATHS.copy()
    try:
        # Replace first path with our temp file
        _CONFIG_PATHS.clear()
        _CONFIG_PATHS.append(config_path)
        # Reset cached config by importing the module and setting the variable
        import utils
        ...
        # Restore
        _CONFIG_PATHS.clear()
        _CONFIG_PATHS.extend(original_paths)
